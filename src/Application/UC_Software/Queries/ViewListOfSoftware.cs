...
public class ViewListOfSoftware
{
    ...
    public class Handler(IVitomDbContext context, ICacheServices cacheServices)
        : IRequestHandler<Query, Result<PaginatedResponse<SoftwareDetailsResponse>>>
    {
        public async Task<Result<PaginatedResponse<SoftwareDetailsResponse>>> Handle(
            Query request,
        ...
        )
        {
            // set key
            string key =
                $"software-keyword{request.Keyword}-pagesize{request.PageSize}-pageindex-{request.PageIndex}";
            // get cache result
            PaginatedResponse<SoftwareDetailsResponse>? cacheResult = await cacheServices.GetAsync<
                PaginatedResponse<SoftwareDetailsResponse>
            ...
            IQueryable<Software> query = context
                ...
                .Where(s => s.IsNotDeleted());
            // calculating total pages
            int totalPages = (int)Math.Ceiling((decimal)query.Count() / request.PageSize);
            // get result
            IEnumerable<SoftwareDetailsResponse> result = await query
